...
class SegDirectoryIterator(Iterator):
    ...
    label_suffix: label file suffix, such as `.png`, or `.pkl`
    ...
    def __init__(self, file_path, seg_data_generator,
                 ...
                 label_dir, label_suffix, classes, ignore_label=0,
                 crop_mode='none', label_cval=7, pad_size=None,
        ...
        white_list_formats = {'png', 'jpg', 'jpeg', 'bmp', 'pkl'}

        # build lists for data files and label files
        self.class_suffix = '_label.pkl'
        self.data_files = []
        self.label_files = []
        fp = open(file_path)
        lines = fp.readlines()
        ...
        for line in lines:
            ...
            self.label_files.append(line + self.class_suffix)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            #if self.label_file_format == 'pkl':
            with open(label_filepath,'rb') as f:
                y = pkl.load(f)
                y = np.reshape(y,(512,512,1))
            '''
            else:
            ...
            if self.target_size:
                if self.crop_mode != 'none':
                    ...
                    #if self.label_file_format is not 'pkl':
                    #    y = img_to_array(
                    ...
                    # print("1 y.shape = {}".format(y.shape))
                    #else:
                    #   print('ERROR: resize not implemented for label pkl file')
