...
def AtrousFCN_Vgg16_16s(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
    ...
    top_model.add(Dropout(0.5))
    top_model.add(Conv2D(4096, (1, 1), activation='relu', padding='same', dilation_rate=(2,2), name='fc2', kernel_regularizer=l2(weight_decay)))
...
def AtrousFCN_Vgg16_16s_adad(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
...
def AtrousFCN_Vgg16_16s_aad(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
...
def AtrousFCN_Vgg16_16s_aa(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
    ...
    return new_model
'''
def AtrousFCN_Resnet50_16s(input_shape = None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
    ...
    x = atrous_conv_block(3, [512, 512, 2048], stage=5, block='a', weight_decay=weight_decay, atrous_rate=(1, 1), batch_momentum=batch_momentum)(x)
    x = atrous_identity_block(3, [512, 512, 2048], stage=5, block='b', weight_decay=weight_decay, atrous_rate=(1, 1), batch_momentum=batch_momentum)(x)
    x = atrous_identity_block(3, [512, 512, 2048], stage=5, block='c', weight_decay=weight_decay, atrous_rate=(1, 1), batch_momentum=batch_momentum)(x)
    model = Model(img_input, x)
    model.summary()
    current_dir = os.path.dirname(os.path.realpath(__file__))
    weights_path = os.path.join(current_dir,'weight/resnet50_weights_tf_dim_ordering_tf_kernels.h5') 
    model.load_weights(weights_path, by_name=True)
    ...
    #classifying layer
    top_model = Sequential()
    top_model.add(Conv2D(classes, (3, 3), dilation_rate=(2, 2), kernel_initializer='normal', activation='linear', padding='same', strides=(1, 1), kernel_regularizer=l2(weight_decay)))
    top_model.add(Conv2D(classes, (1, 1), kernel_initializer='he_normal', activation='linear', padding='same', strides=(1, 1), kernel_regularizer=l2(weight_decay)))
    top_model.add(Conv2DTranspose(filters = classes, kernel_size = (32, 32), strides = (16, 16), padding='same'))
    new_model.add(top_model) 
    top_model.summary()
...
def FCN_Vgg16_8s(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
    ...
    # o
    o = f5 
    o = (Conv2D(4096,(7,7),activation='relu',padding='same',name='fcn_conv1'),kernel_regularizer=l2(weight_decay))(o)
    o = (Conv2D(4096,(1,1),activation='relu',padding='same',name='fcn_conv1'),kernel_regularizer=l2(weight_decay))(o)
    o = (Conv2D(classes, (1, 1), kernel_initializer='he_normal', activation='linear', padding='valid', strides=(1, 1), kernel_regularizer=l2(weight_decay)))(o)
    o = (Conv2DTranspose(filters = classes, kernel_size = (4, 4), strides = (2, 2), padding='same'))(o)
    
    # o2
    o2 = f4
    o2 = (Conv2D(classes, (1, 1), kernel_initializer='he_normal', activation='linear', padding='valid', kernel_size = (4,4), strides=(1, 1), kernel_regularizer=l2(weight_decay)))(o2)
    # New sequential model for the convenience of concatenating
    new_model = Sequential()
    for layer in model.layers:
      new_model.add(layer)
...
def FCN_Resnet50_32s(input_shape = None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=8):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
...
def DenseNet_FCN(input_shape=None, weight_decay=1E-4,
                 batch_momentum=0.9, batch_shape=None, classes=7,
                 include_top=False, activation='sigmoid'):
    if include_top is True:
        # TODO(ahundt) Softmax is pre-applied, so need different train, inference, evaluate.
        # TODO(ahundt) for multi-label try per class sigmoid top as follows:
